...
class GeminiProvider implements AiProviderInterface
{
    ...
    public function generateContent(JobPost $job): array
    {
        if (empty($this->apiKey)) {
            throw new \Exception("Gemini API key is missing. Please configure GEMINI_API_KEY.");
        }

        $prompt = $this->buildPrompt($job);

        Log::info("Dispatching Gemini content generation for Job ID: {$job->id}");
        ...
        if (!$response->successful()) {
            ...
            throw new \Exception("Gemini returned error: {$error}");
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            throw new \Exception("Gemini output was not valid JSON: " . json_last_error_msg());
        }
